<template>
  <div class="fixed top-0 bg-neutral-900 lg:bg-gray-900 text-white flex flex-wrap lg:flex-nowrap items-center justify-between py-2 px-4 md:py-4 md:px-8 w-full">
    ...
    <button class="block lg:hidden" @click="navOpened = !navOpened">
      <image-svg type="menu" class="h-8 w-8" />
    </button>
    <nav class="text-2xl flex flex-col lg:!block">
      ...
      <button class="inline-block p-4"
              @click="navOpened = false; $refs.contact.open()"
      >
        Nous contacter
      </button>
    </nav>
  ...
  <teleport to="body">
    <page-contact ref="contact" />
  </teleport>
</template>

<script setup lang="ts">
const links = [
  {id: 'header', 'text': 'Accueil'},
  {id: 'talks', 'text': 'Programmation'},
  {id: 'speakers', 'text': 'Nos speakers'},
];
